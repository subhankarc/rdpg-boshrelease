...
func (p *PG) DisableDatabase(dbname string) (err error) {
	log.Trace(fmt.Sprintf(`pg.PG<%s>#DisableDatabase(%s) Disabling postgres database...`, p.IP, dbname))
	p.Set(`database`, `postgres`)
	db, err := p.Connect()
	if err != nil {
	...
	_, err = db.Exec(sq)
	if err != nil {
		log.Error(fmt.Sprintf("pg.PG<%s>#DisableDatabase(%s) DISABLE ! %s", p.IP, dbname, err))
	}
	return
}
...
func (p *PG) Connect() (db *sqlx.DB, err error) {
	log.Trace(fmt.Sprintf(`pg.PG<%s>#Connect() %+v`, p.IP, p.URI))
...
func (p *PG) WaitForRegClass(k string) (err error) {
	...
	db, err := p.Connect()
	if err != nil {
		log.Error(fmt.Sprintf("tasks.Scheduler() Failed connecting to %s err: %s", p.URI, err))
		return err
	}
	defer db.Close()
